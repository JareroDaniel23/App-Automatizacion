...
export default function ReportTable({ data, eliminarVehiculo,  }) {
  ...
  const totalAgua = useMemo(() => calcularTotal(data, 'agua'), [data]);

  const totalDesinfectante = useMemo(() => calcularTotal(data, 'desinfectante'), [data]);
  const totalDesengrasante = useMemo(() => calcularTotal(data, 'desengrasante'),[data]);
  const totalBlanqueador = useMemo(() => calcularTotal(data, 'blanqueador'), [data]);
   if (!data) return null;
